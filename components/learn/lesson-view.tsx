...
type Step = 'concept' | 'code' | 'practice' | 'quiz' | 'done'

export default function LessonView({ lesson, taskId, taskLabel, onComplete }: Props) {
  const hasPractice = !!lesson.practice

  const steps: Step[] = hasPractice
    ? ['concept', 'code', 'practice', 'quiz', 'done']
    : ['concept', 'code', 'quiz', 'done']
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 20, overflowX: 'auto', paddingBottom: 2 }}>
        {steps.filter(s => s !== 'done').map((s, i) => {
          ...
          const stepColor = s === 'practice' ? 'var(--c-03)' : 'var(--primary)'
          return (
            <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
              <div style={{
                width: 30, height: 30, borderRadius: '50%',
                border: `2px solid ${isDone ? 'var(--c-05)' : isActive ? stepColor : 'var(--line)'}`,
                background: isDone
                  ? 'color-mix(in oklab, var(--c-05) 16%, var(--surface))'
                  : isActive
                    ? `color-mix(in oklab, ${stepColor} 12%, var(--surface))`
                    : 'var(--surface)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 11, fontWeight: 800,
                color: isDone ? 'var(--c-05)' : isActive ? stepColor : 'var(--ink-mute)',
                boxShadow: isActive ? `0 2px 0 color-mix(in oklab, ${stepColor} 60%, black)` : isDone ? '0 2px 0 var(--c-05-d)' : '0 2px 0 var(--line-2)',
                transition: 'all 0.2s',
              }}>
                {isDone ? '✓' : i + 1}
              </div>
              <span style={{ fontSize: 10, fontWeight: isActive ? 800 : 600, color: isActive ? 'var(--ink)' : 'var(--ink-mute)', letterSpacing: '0.04em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                {stepLabels[s]}
              </span>
              {i < steps.filter(s => s !== 'done').length - 1 && (
                <div style={{ width: 20, height: 2, background: isDone ? 'var(--c-05)' : 'var(--line)', borderRadius: 2 }} />
              )}
            </div>
          )
        })}
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--ink-mute)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 3 }}>
          Leçon en cours
        </div>
      ...
      {step === 'concept' && (
        <div>
          <div style={card}>
            <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--c-05)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8 }}>📖 Concept</div>
            <p style={{ fontSize: 14, color: 'var(--ink)', lineHeight: 1.65, margin: 0, fontWeight: 600 }}>{lesson.concept}</p>
          </div>
          <div style={card}>
            <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--primary)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8 }}>🎯 Pourquoi c&apos;est important</div>
            <p style={{ fontSize: 14, color: 'var(--ink)', lineHeight: 1.65, margin: 0, fontWeight: 600 }}>{lesson.why_it_matters}</p>
          </div>
          <div style={card}>
            <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--gem-d)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8 }}>💡 Pro tip</div>
      ...
      {step === 'code' && (
        <div>
          <div style={card}>
            <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--c-03)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 12 }}>
              {'</>'} Exemple {lesson.code_language}
            </div>
            <pre style={{
              background: '#161525', border: '2px solid rgba(255,255,255,0.08)',
              borderRadius: 'var(--r-md)', padding: '16px 18px',
              fontFamily: 'var(--f-mono)', fontSize: 12.5, color: 'rgba(255,255,255,0.85)',
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <button
              className="btn-3d"
              onClick={() => setStep(hasPractice ? 'practice' : 'quiz')}
              style={hasPractice ? { '--btn-c': 'var(--c-03)', '--btn-d': 'var(--c-03-d)' } as React.CSSProperties : undefined}
            >
              {hasPractice ? '⚡ Pratiquer →' : 'Quiz →'}
            </button>
          </div>
        </div>
      )}
      ...
      {step === 'quiz' && (
        <div>
          <div style={card}>
            <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--flame)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 10 }}>🧠 Quiz</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {lesson.quiz.options.map((option, idx) => {
                ...
                if (quizResult) {
                  ...
                  borderColor = 'var(--primary)'; bg = 'color-mix(in oklab, var(--primary) 8%, var(--surface))'; shadow = '0 2px 0 var(--primary-dark)'
                }
                return (
                  <button
                    key={idx}
                    ...
                    style={{
                      ...
                      boxShadow: shadow, fontFamily: 'var(--f-sans)', transition: 'all 0.1s',
                    }}
                  >
                    <span style={{
          ...
          {quizResult && (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontFamily: 'var(--f-sans)', fontWeight: 900, fontSize: 15, color: quizResult.correct ? 'var(--c-05)' : 'var(--c-04)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <button
              onClick={() => setStep(hasPractice ? 'practice' : 'code')}
              style={{
                padding: '10px 20px', background: 'var(--surface)', border: '2px solid var(--line)',
                borderRadius: 99, cursor: 'pointer', fontSize: 13, fontWeight: 800,
                color: 'var(--ink-2)', fontFamily: 'var(--f-sans)', boxShadow: '0 3px 0 var(--line-2)',
      ...
      {step === 'done' && (
        <div style={{ textAlign: 'center' }}>
          <div style={{
            ...
            padding: '36px 28px', boxShadow: '0 4px 0 var(--gold-d)', marginBottom: 14,
          }}>
            <div style={{ margin: '0 auto 14px', animation: 'crown-pop 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)', display: 'inline-block' }}>
              <FennecMascot size={80} mood="happy" />
            </div>
            <h3 style={{ fontFamily: 'var(--f-sans)', fontSize: 22, fontWeight: 900, color: 'var(--ink)', marginBottom: 6, letterSpacing: '-0.02em' }}>
              Leçon terminée !
            </h3>
            <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-3)', marginBottom: 0 }}>
              &quot;{taskLabel}&quot;
            </p>
          </div>
          <div style={{
            ...card, textAlign: 'left', marginBottom: 14,
          }}>
            <div style={{ fontSize: 10, fontWeight: 800, color: 'var(--primary)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8 }}>
              🚀 Prochaine étape
            </div>
